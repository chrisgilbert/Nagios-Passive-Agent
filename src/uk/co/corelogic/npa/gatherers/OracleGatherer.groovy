...
final String tbs_sql="""
    ...
    private MetricModel getDoubleModel(String name, String identifier){
      ...
      */
     private MetricGroup getMetricGroup(String name) {
        MetricGroup grp = new MetricGroup(name, this.initiatorID, this.tbsMetricList)
        return grp
     }
    ...
    */
    ...
    if ( val == null ) {
        ...
        getTablespaceMetrics(mod.clone(), grp)
    ...
    private ORA_TBS_MB_FREE(variables) {
        ...
        def mod = this.getDoubleModel("ORA_TBS_MB_FREE", identifier)

        // Define group identifiers to ensure efficient database retrieval
        String uniqueName = "ORATBS_" + mod.identifier
        MetricGroup grp = getMetricGroup(uniqueName)
    ...
    private ORA_TBS_MB_USED(variables) {
        // Configure standard metric variables
        def identifier = variables.tablespace_name
        def mod = this.getDoubleModel("ORA_TBS_MB_USED", identifier)
    ...
    private ORA_TBS_MB_TOTAL(variables) {
        // Configure standard metric variables
        def identifier = variables.tablespace_name
        def mod = this.getDoubleModel("ORA_TBS_MB_TOTAL", identifier)
    ...
    private double ORA_STATS_BUFFER_CACHE_HIT_RATIO(variables) {
        ...
        def mod = this.getDoubleModel("ORA_STATS_BUFFER_CACHE_HIT_RATIO", identifier)
        ...
        def met = persistMetric(mod, value.toDouble(), datestamp)
